...
class triplemoji(moji):
    def build_model(self, MAX_DEGREE):
        ...
        # Inputs
        self.chubby_exp = Input(shape=(self.output_dim,))
        self.chubby_P = Input(shape=(self.output_dim,))
        self.chubby_N = Input(shape=(self.output_dim,))
        ...
        z_mean, z_log_var, z = self.dog_encoder(self.chubby_exp)
        code_chubby = z_mean
        #code_chubby = z
        kl_loss = 1 + z_log_var - K.square(z_mean) - K.exp(z_log_var)
        self.kl_loss = K.mean(K.abs(K.sum(kl_loss, axis=-1)))

        z_mean, z_log_var, z = self.dog_encoder(self.chubby_P)
        code_P = z_mean
        #code_P = z

        z_mean, z_log_var, z = self.dog_encoder(self.chubby_N)
        code_N = z_mean
        #code_N = z

        data_chubby = self.dog_decoder(code_chubby)
        ...
        d=0.0
        self.group_loss = K.mean(K.abs((data_chubby - data_people)*ratio)) / 1.8
        ...
        self.re_loss_P = K.mean(K.abs((self.chubby_P - self.dog_gcn_vae_exp(self.chubby_P)) * ratio )) / 1.8 
        self.re_loss_N = K.mean(K.abs((self.chubby_N - self.dog_gcn_vae_exp(self.chubby_N)) * ratio )) / 1.8
        ...
        #self.train_func = K.function([real, self.chubby_exp], [self.group_loss, self.loss, self.regular_loss], training_updates)
        #self.test_func = K.function([real, self.chubby_exp], [self.group_loss, self.loss])
        self.train_func = K.function([real, self.chubby_P, self.chubby_N], [self.tpl_loss, self.positive_loss, self.re_loss, self.loss, self.regular_loss], training_updates)
        self.test_func = K.function([real, self.chubby_P, self.chubby_N], [self.tpl_loss, self.positive_loss, self.re_loss, self.loss])
        
        # Load gcn_vae_exp model, not the splited ones.
        self.gcn_vae_exp.load_weights('model/gcn_vae_exp_chubby_tpl.h5')
    ...
    def test(self, limit=5, filename='test', people_id=142):
        ...
        people_exp = denormalize_fromfile(self.people_decoder.predict(self.people_encoder.predict(data_array, batch_size=self.batch_size)[0],batch_size=self.batch_size), self.p_M_list, self.p_m_list)
        import shutil, os
