...
max_feature = np.load(prefix+"data/people/max_data.npy") + cross_id
min_feature = np.load(prefix+"data/people/min_data.npy") + cross_id
ref_mesh_filename = prefix+"data/people/Mean_Face.obj"  # here is the filename of reference mesh
...
def V2M(array, filename):
    ...
    V = igl.eigen.MatrixXd()
    F = igl.eigen.MatrixXi()
    igl.readOBJ('/raid/jzh/FeatureDistangle/data/distangle/Mean_Face.obj',V,F)
    #igl.readOBJ('/home/jzh/FeatureDistangle/src/Mean_Face.obj',V,F)
...
def V2M2(array, filename, ref_name = '/raid/jzh/FeatureDistangle/data/distangle/Mean_Face.obj', v_num = 11510):
    def p2e(m):
        if isinstance(m, np.ndarray):
            if not (m.flags['C_CONTIGUOUS'] or m.flags['F_CONTIGUOUS']):
                raise TypeError('p2e support either c-order or f-order')
...
def obj2dat_vertex(filename):
    ref_name = '/raid/jzh/FeatureDistangle/data/distangle/Mean_Face.obj'
    V= igl.eigen.MatrixXd()
    F= igl.eigen.MatrixXi()
    igl.readOBJ(ref_name,V,F)
...
if __name__ == '__main__':
    ...
    start = True
    ref_name = '../data/distangle/Mean_Face.obj'
    from get_mesh import get_mesh
    #a = np.load('/home/jzh/FeatureDistangle/data/distangle/train_data/Feature1.npy')
    #generate_mesh(a[2]+cross_id, 'generate_2.obj', False)
    #generate_mesh('/media/jzh/F/data/CropFeature1005/RimdFeature1005_crop/Feature1/1_1005.dat', 'generate_2.obj', False)
    for i in range(2000):
        print(i)
        #obj2dat_vertex('/raid/jzh/FeatureDistangle/data/xyz/Interpolated_results/aligned_interpolated_{}.obj'.format(i))
        data = np.fromfile('/raid/jzh/FeatureDistangle/data/xyz/Interpolated_results/aligned_interpolated_{}.dat'.format(i))
        data.tofile('/raid/jzh/FeatureDistangle/data/xyz/Interpolated_results/interpolated_{}.dat'.format(i))
#    for i in range(200):
#        data = np.fromfile('/raid/jzh/Extrapolated_results/Extrapolated_results_141/extrapolated_{}.dat'.format(i))
#        ##print(data.shape)
#        #print(get_mesh(ref_name, data))
#        V2M2(get_mesh(ref_name, data), ('/raid/jzh/Extrapolated_results/extrapolated_141_{}.obj').format(i))
