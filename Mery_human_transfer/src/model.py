...
class triplemoji(moji):
    def build_model(self, MAX_DEGREE):
        ...
        # Inputs
        self.mery_exp = Input(shape=(self.output_dim,))
        self.mery_P = Input(shape=(self.output_dim,))
        self.mery_N = Input(shape=(self.output_dim,))
        ...
        z_mean, z_log_var, z = self.dog_encoder(self.mery_exp)
        code_mery = z_mean
        #code_mery = z
        kl_loss = 1 + z_log_var - K.square(z_mean) - K.exp(z_log_var)
        self.kl_loss = K.mean(K.abs(K.sum(kl_loss, axis=-1)))

        z_mean, z_log_var, z = self.dog_encoder(self.mery_P)
        code_P = z_mean
        #code_P = z

        z_mean, z_log_var, z = self.dog_encoder(self.mery_N)
        code_N = z_mean
        #code_N = z

        data_mery = self.dog_decoder(code_mery)
        ...
        d=0.0
        self.group_loss = K.mean(K.abs((data_mery - data_people)*ratio)) / 1.8
        ...
        self.re_loss_P = K.mean(K.abs((self.mery_P - self.dog_gcn_vae_exp(self.mery_P)) * ratio )) / 1.8 
        self.re_loss_N = K.mean(K.abs((self.mery_N - self.dog_gcn_vae_exp(self.mery_N)) * ratio )) / 1.8
        ...
        #self.train_func = K.function([real, self.mery_exp], [self.group_loss, self.loss, self.regular_loss], training_updates)
        #self.test_func = K.function([real, self.mery_exp], [self.group_loss, self.loss])
        self.train_func = K.function([real, self.mery_P, self.mery_N], [self.tpl_loss, self.positive_loss, self.re_loss, self.loss, self.regular_loss], training_updates)
        self.test_func = K.function([real, self.mery_P, self.mery_N], [self.tpl_loss, self.positive_loss, self.re_loss, self.loss])

        # Load gcn_vae_exp model, not the splited ones.
        self.gcn_vae_exp.load_weights('model/gcn_vae_exp_mery_tpl.h5')
