...
void RIMD_Reconstruction::InterlateRIMD(double t, std::string _filename)
{
    ...
    {
        Eigen::Matrix3d logdR = ref_mesh_.property(log_dRs,*he_it);
        ref_mesh_.property(log_dRs,*he_it) = (1.0-t)*zero + t*logdR;
    }
    check_RIMD_correct();
    std::cout<<"This mesh has " << v_num << " points."<<std::endl;

    // Store S and logR feature point-by-point.
    // For each vertex v,
    // S matrix of v could be extracted as a 6-dims vector (S matrix is symmetric).
    // logR matrix of v could be extracted as 3-dims vector (logR matrix is skew symmetric).
    // We simply combine these 9-dims information of each vertex together and then insert them to a 1*(9|V|) vector.
    double feature[v_num*9];
    int id=0;
    for(TriMesh::VertexIter v_it=ref_mesh_.vertices_begin(); v_it!=ref_mesh_.vertices_end(); v_it++)
    {
        ...
        Eigen::Matrix3d logR = ref_mesh_.property(log_R, *v_it);
        //std::cout<<"Scale "<<std::endl<<Scale<<std::endl;
        //std::cout<<"logR "<<std::endl<<logR<<std::endl;

        // Scale matrix part.
        feature[id*9+0] = Scale(0,0);
        feature[id*9+1] = Scale(0,1);
        feature[id*9+2] = Scale(0,2);
        feature[id*9+3] = Scale(1,1);
        feature[id*9+4] = Scale(1,2);
        feature[id*9+5] = Scale(2,2);

        // logR matrix part.
        feature[id*9+6] = logR(0,1);
        feature[id*9+7] = logR(0,2);
        feature[id*9+8] = logR(1,2);
    ...
    f.write((char*)feature, v_num*9*sizeof(double));
    f.close();
}

void RIMD_Reconstruction::LoadRIMD(std::string _filename)
{
    int LOAD_LOGDR_DATA=0;           // Note that we have two kinds of feature(data) expressions: log_dR and log_R.

    // Load RimdFeature in log_dR type.
    if(LOAD_LOGDR_DATA){
        double rimd_feature[11510*9];
        ...
        r.read((char*)rimd_feature,11510*9*sizeof(double));
        int i=0;
        for(TriMesh::VertexIter v_it = ref_mesh_.vertices_begin();v_it!=ref_mesh_.vertices_end();v_it++){
    ...
    // Load RimdFeature in log_R type.
    else {
        double rimd_feature[11510*9];
        std::ifstream r(_filename, std::ios::binary);
        if(!r)
        ...
        for(TriMesh::VertexIter v_it = ref_mesh_.vertices_begin();v_it!=ref_mesh_.vertices_end();v_it++){
            ...
            logR(0, 0)=0;
            logR(0, 1)=rimd_feature[i+6];
            logR(0, 2)=rimd_feature[i+7];
            logR(1, 0)=-rimd_feature[i+6];
            logR(1, 1)=0;
            logR(1, 2)=rimd_feature[i+8];
            logR(2, 0)=-rimd_feature[i+7];
            logR(2, 1)=-rimd_feature[i+8];
            logR(2, 2)=0;

            ref_mesh_.property(scaling_matrixs, *v_it) = Scale;
            ref_mesh_.property(log_R, *v_it) = logR;
            i=i+9;
        }
        ...
        for(TriMesh::HalfedgeIter h_it=ref_mesh_.halfedges_begin();h_it!=ref_mesh_.halfedges_end();h_it++)
        {
            ...
            Eigen::Matrix3d log_Ri = ref_mesh_.property(log_R,from_vertex);
            Eigen::Matrix3d log_Rj = ref_mesh_.property(log_R,to_vertex);
            Eigen::Matrix3d Ri = rotation_log_exp::exp(log_Ri);
            Eigen::Matrix3d Rj = rotation_log_exp::exp(log_Rj);
...
void RIMD_Reconstruction::compute_Spatial_of_ref_to_defor(std::string _filename)
{
    double Spatial_feature[9499*3];

    TriMesh::VertexIter ref_v_it = ref_mesh_.vertices_begin();
    TriMesh::VertexIter def_v_it = defor_mesh_.vertices_begin();

    int i=0;
    while(ref_v_it != ref_mesh_.vertices_end() && def_v_it != defor_mesh_.vertices_end()){
        TriMesh::Point defor_point = defor_mesh_.point(def_v_it.handle());
        TriMesh::Point ref_point = ref_mesh_.point(ref_v_it.handle());
        //printf("%f\n",ref_point[0]);
        //printf("%f\n",ref_point[1]);
        //printf("%f\n",ref_point[2]);
        TriMesh::Point delta = defor_point - ref_point;
        for(int k=0;k<3;k++){
            //if(delta[k]>1e-5)
                Spatial_feature[i+k] = delta[k];
            //else
                //Spatial_feature[i+k] = 0;
        }
...
//  //      printf("%f\n ", Spatial_feature[j]);
    ...
    f.write((char*)Spatial_feature, 9499*3*sizeof(double));
    f.close();
...
void RIMD_Reconstruction::compute_ref_LB_weights()
{
    ...
    for (e_it=ref_mesh_.edges_begin(); e_it!=e_end; ++e_it)
    {
        ...
        if(ref_mesh_.is_boundary(*e_it))
        {
            ...
            // Debug
            //ref_mesh_.property(LB_weights,e_it) = 2.0;
            continue;
        }
        h0 = ref_mesh_.halfedge_handle(e_it.handle(), 0);
        v0 = ref_mesh_.to_vertex_handle(h0);
...
void RIMD_Reconstruction::compute_logR_matrixs()
{
    TriMesh::VertexIter v_it;
    for(v_it=ref_mesh_.vertices_begin(); v_it!=ref_mesh_.vertices_end(); v_it++)
    {
        Eigen::Matrix3d R = ref_mesh_.property(rotation_matrixs,*v_it);
        Eigen::Matrix3d logR = rotation_log_exp::log(R);
        for(int i =0;i<3;i++)
            for(int j=0; j<3;j++)
        if(std::isnan(logR(i,j)))
            std::cout<<"logR fx"<<i<<j<<std::endl;
        ref_mesh_.property(log_R,*v_it) = logR;
    }
}
...
void RIMD_Reconstruction::compute_internal_for_globalstep()
{
    ...
    for(;v_it!=ref_mesh_.vertices_end();v_it++)
    {
        ...
        for(;ve_it.is_valid();ve_it++)
        {
            ...
            Eigen::Matrix3d R=rotation_log_exp::exp(ref_mesh_.property(log_dRs,h_e));
//            if(std::isnan(R(0,0)))
//                std::cout<<"dasdsa";

        }
//        if(std::isnan(temp(0,0)))
